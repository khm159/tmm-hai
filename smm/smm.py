...
class SMM:
    ...
    def __init__(self, model:str, visibility:str, agent:str):
        ...
        # if you would like to create your own mental models, you can add them here
        else:
            raise ValueError("Failed to initialize SMM, the only supported model is 'predicates'")
    ...
    def init_belief_state_from_file(self, layout:str):
        if not layout.endswith(".layout"):
            layout += ".layout"
        with open(f"env/server/layouts/{layout}", "r") as f:
    ...
    def filter_visibility(self, state:dict):
        ...
        if False:  # in this work we did not filter out other agents, as it was assumed an agent could reasonably hear where another agent would be located
