...
function graphics_end() {
  if (graphics == undefined) {
    console.log("graphics_end(), graphics variable is undefined!");
  } else {
...
class GraphicsManager {
  constructor(game_config, scene_config, graphics_config) {
    ...
    game_config.height =
      scene_config.tileSize * scene_config.terrain.length +
      scene_config.hud_size;
...
class OvercookedScene extends Phaser.Scene {
  constructor(config) {
    ...
    this.visibility = config.start_state.visibility;
  ...
  preload() {
    ...
    this.load.atlas(
      "visibility",
      this.assets_loc + "visibility.png",
      this.assets_loc + "visibility.json",
    );
  }
  ...
  drawLevel() {
    ...
    for (let row in pos_dict) {
      if (!pos_dict.hasOwnProperty(row)) {
        continue;
      }
  ...
  _drawState(state, sprites) {
    ...
    sprites["objects"] = {};
    let playerIndex = state.players.length - 1; // Jack: always showing user as the last agent

    for (let objpos in state.objects) {
      if (!state.objects.hasOwnProperty(objpos)) {
        continue;
      }
      let obj = state.objects[objpos];
      let [x, y] = obj.position;
      // ignore if not visible to P2 (index 1) (user) OR if the game is paused
      if (!state.visibility[y][x][playerIndex] || paused) {
        continue;
      }
      ...
      if (obj.name === "soup" && terrain_type === "P") {
        ...
        let show_time = true;
        if (
          (obj._cooking_tick > obj.cook_time && !this.show_post_cook_time) ||
          obj._cooking_tick == -1
        ) {
          show_time = false;
        }
    ...
    sprites["indicators"] =
      typeof sprites["indicators"] === "undefined" ? {} : sprites["indicators"];
    if (visibilityDots) {
      for (let y in state.visibility) {
        for (let x in state.visibility[y]) {
          if (!(y + "," + x in sprites["indicators"])) {
            let objsprite = this.add.sprite(
              this.tileSize * x,
              this.tileSize * y,
              "visibility",
          ...
          // }
          // player sees
          else if (state.visibility[y][x][playerIndex]) {
  ...
  _drawHUD(hud_data, sprites, board_height, width) {
    // Jack: not showing all orders
    if (false && typeof hud_data.all_orders !== "undefined") {
      this._drawAllOrders(hud_data.all_orders, sprites, board_height);
    }
    // Jack: not showing bonus orders
    if (false && typeof hud_data.bonus_orders !== "undefined") {
    ...
    // Jack: changing this to completed dishes
    if (typeof hud_data.score !== "undefined") {
      this._drawScore(hud_data.score, sprites, board_height);
    }
    if (
      typeof hud_data.potential !== "undefined" &&
      hud_data.potential !== null
    ) {
  ...
  _drawInstructions(sprites, board_height, width) {
    if (typeof sprites["instructions"] == "undefined") {
      sprites["instructions"] = this.add.text(
        150,
        board_height + 30,
    ...
    if (typeof orders !== "undefined" && orders !== null) {
      ...
      if (typeof sprites["bonus_orders"] !== "undefined") {
        ...
        for (let i = 0; i < orders.length; i++) {
          let spriteFrame = this._ingredientsToSpriteFrame(
  ...
  _drawScore(score, sprites, board_height) {
    score = "Dishes: " + Math.ceil(score);
    if (typeof sprites["score"] !== "undefined") {
      sprites["score"].setText(score);
    } else {
      sprites["score"] = this.add.text(5, board_height + 90, score, {
        font: "20px Arial",
        fill: "red",
        align: "left",
      });
  ...
  _drawPotential(potential, sprites, board_height) {
    ...
    if (typeof sprites["potential"] !== "undefined") {
      ...
      sprites["potential"] = this.add.text(100, board_height + 90, potential, {
        font: "20px Arial",
        fill: "red",
        align: "left",
      });
  ...
  _drawTimeLeft(time_left, sprites, board_height) {
    ...
    if (typeof sprites["time_left"] !== "undefined") {
      ...
      sprites["time_left"] = this.add.text(5, board_height + 115, time_left, {
        font: "20px Arial",
        fill: "red",
        align: "left",
      });
