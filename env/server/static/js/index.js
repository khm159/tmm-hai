...
function displaySMM(smm) {
  ...
  let s = JSON.stringify(smm, null, 4)
    .replace(/[{},'"[\]]/g, "")
    .replace(/(^[ \t]*\n)/gm, "")
    .replace(/^(    +)/gm, (match, tabs) => tabs.replace(/    /, ""));
  document.getElementById("smm-content").innerHTML =
    "<b>- Mental Model -</b>" + "<pre>" + s + "</pre>";
}
...
function highlightStudyStage(s) {
  studyStages.forEach((i) => {
    ...
    document.getElementById("stage-" + i).className =
      i == s ? "study-stage study-stage-active" : "study-stage";
  });
}
...
async function setStudyStage(s, indicator = "") {
  ...
  if (
    ["intro", "practice", "round1", "round2", "round3", "round4"].includes(
      studyStage,
    )
  ) {
    showOvercooked();
...
function showInstructions(text = "") {
  ...
  document
    ...
    .setAttribute("class", "instructions-content");
...
function introduceStage() {
  ...
  if (window.mobileAndTabletCheck()) {
    showInstructions(
      "Hi! We would love for you to participate in our study, but it looks like you are using a phone or tablet. This study requires a laptop or desktop computer -- you can click below to return to Prolific without penalty.",
    );
    ...
    document.getElementById("instructions-continue-text").innerHTML =
      "End Study";
  }
  ...
  if (studyStage == "practice") {
    ...
    showInstructions(
      'Well done! Let\'s do one more practice, try to cook all the soups before time runs out. This time, you will be asked a few questions every 30 seconds. Press "Continue", take in in the environment, and then "Play"',
    );
    setInstructionsButtonToContinue(
      undefined,
      () => {
        previewRound("preview_kitchen_practice2.png", "practice");
      },
      1,
    );
    return;
  ...
  if (studyStage == "round1") {
    // show the instructions
    showInstructions(
      'Great! You are ready for the real deal. Press "Continue", take in the environment, and go for it.',
    );
    setInstructionsButtonToContinue(
      undefined,
      () => {
        previewRound("preview_kitchen_round1.png", "round1");
      },
      5,
    );
    return;
  ...
  if (studyStage == "round2") {
    // show the instructions
    showInstructions(
      "Nice work! Let's pause a few seconds before starting the second round.",
    );
    setInstructionsButtonToContinue(
      undefined,
      () => {
        previewRound("preview_kitchen_round2.png", "round2");
      },
      5,
    );
    return;
  ...
  if (studyStage == "round3") {
    // show the instructions
    showInstructions(
      "You are halfway done! We will pause for a few seconds before starting the next round.",
    );
    setInstructionsButtonToContinue(
      undefined,
      () => {
        previewRound("preview_kitchen_round3.png", "round3");
      },
      5,
    );
    return;
  ...
  if (studyStage == "round4") {
    // show the instructions
    showInstructions(
      "Just one more, you got this! Take some deep breaths to relax.",
    );
    setInstructionsButtonToContinue(
      undefined,
      () => {
        previewRound("preview_kitchen_round4.png", "round4");
      },
      5,
    );
    return;
...
function showInstructions1Text() {
  showInstructions(
    "This is your chef. Cute, right?<br><br><img height='150rem' src='static/images/chef.png'/><br>To play the game, control your chef with the arrow keys or WASD keys.<br><br><img class='instructions-controls-image' src='static/images/controls.png'/><br>",
  );
  setInstructionsButtonToContinue(
    showIntroductionText,
    showInstructions2Text,
    3,
  );
}
...
function showConsent() {
  showInstructions(
    "Please review the consent form. If you consent to the study, please enter your Prolific ID in the text box. If not, you can close this tab and return the study with the code <b style='user-select:text'>CIQE9XIY</b> for no penalty.",
  );
  ...
  setInstructionsButtonToContinue(
    ...
    () => {
      ...
      if (
        !["", ".", ",", "<", ">", ".."].includes(
          document.getElementById("consent-input").value.trim(),
        )
      ) {
        ...
        alert(
          "If you consent to the study, please enter your Prolific ID in the text field!",
        );
      }
    },
    1,
...
function showScreeningInfo() {
  showInstructions(
    "Let's start with a few screening questions to determine if you are eligible for this study.",
  );
...
function showScreeningAge() {
  ...
  setInstructionsButtonToContinue(
    ...
    () => {
      ...
      if (
        !isNaN(document.getElementById("screening-age-input").value) &&
        document.getElementById("screening-age-input").value >= 18
      ) {
...
function showScreeningVulnerable() {
  showInstructions(
    "Are you part of a vulnerable group or are otherwise unable to provide consent for a research study? Vulnerable groups include prisoners, mentally disabled persons, and economically or educationally disadvantaged persons.",
  );
...
function showScreeningExit() {
  ...
  showInstructions(
    "We are sorry, but your responses make you ineligible for this study. Thank you for your participation, you can close this tab and return the task with the code <b style='user-select:text'>CIQE9XIY</b> for no penalty",
  );
  hideDemographics();
  // disable the instructions continue button
...
function showDemographicsGamingText() {
  showInstructions(
    "What is your experience with fast-paced team coordination video games, for example, Overcooked, League of Legends, Black Ops?",
  );
  document.getElementById("demographics-gaming").style.display = "flex";
  setInstructionsButtonToContinue(
    showDemographicsIntro,
    showInstructions3Text,
    1,
  );
}
...
function showInstructions3Text() {
  showInstructions(
    'Awesome! Let\'s get started with a practice round! Press "Continue", take in the environment, and then press "Play".<br><br><b><u>As a reminder, you can mix onions and tomatoes in your soups.</u></b>',
  );
  setInstructionsButtonToContinue(
    showDemographicsGamingText,
    () => {
      previewRound("preview_kitchen_practice.png", "intro");
    },
    1,
  );
}
...
function setInstructionsButtonToPlay(nextStep) {
  ...
  document
    ...
    .setAttribute("class", "instructions-play");
  document.getElementById("instructions-continue").onclick = nextStep;
  document.getElementById("instructions-continue-bar").style.backgroundColor =
    "green";
  hideDemographics();
...
function setInstructionsButtonToContinue(prevStep, nextStep, loadingDuration) {
  ...
  document
    .getElementById("instructions-continue")
    .setAttribute("class", "instructions-continue");
  document.getElementById("instructions-continue-bar").style.backgroundColor =
    "blue";
  // init the loading bar
...
function setInstructionsButtonLoading(timeout, after) {
  ...
  const loading = window.setInterval(() => {
    current = (100 * (new Date().getTime() - initTime)) / timeout;
    document.getElementById("instructions-continue-bar").style.width =
      current + "%";
    if (current >= 100) {
      document.getElementById("instructions-continue").style.backgroundColor =
        "";
...
function previewRound(img, stage) {
  ...
  document.getElementById("instructions-content-text-top").innerHTML =
    "<img class='preview-round' src='static/images/" +
    img +
