...
3. The *logical predicates* and *LLM* mental model implementations. Refer to (Mental Models)[#MentalModels] and the paper for details on the models.
...
* `extract_results.py` extracts the metrics of interest from the grader across all participants and stores that data as pickle files. The script has an argument for the AI agents visibility when constructing the mental models, e.g., `V4`.

* `visualize_results.py` uses the pickle files generated by `extract_results.py` to compute and generate the plots shown in the paper.

Script in `plots/` are used by `visualize_results.py` for generating the plots.
